...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) # 64, 75, 75
        x = self.layer1(x) # 256, 75, 75
        feat_s = x
        x = self.layer2(x) # 512, 38, 38
        feat_m = x
        x = self.layer3(x) # 1024, 19, 19
        feat_d = x
        x = self.layer4(x) # 2048, 10, 10
        feat_ds = x
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc(x)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.decoder0 = nn.Sequential(
            ...
            SeparableConv2d(2048, 1024, 3, 1, 1, bias=False),
            nn.BatchNorm2d(1024),
            nn.ReLU()
        )
